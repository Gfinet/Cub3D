...
/*   Updated: 2024/07/31 21:55:48 by Gfinet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculate_perp_wall_dist(t_rcdata *data, int mirr)
{
	...
	{
		if (data->side == 0 || data->side == 2)
			(*data).d_perp_wall_dist = (data->side_dist.x - data->delta_dist.x);
		else if (data->side == 1 || data->side == 3)
			(*data).d_perp_wall_dist = (data->side_dist.y - data->delta_dist.y);
	}
}
		...
		t_cube *cube)
{
	...
	(*dr).tex_x = (int)((dr->wall_x) * (double)(dr->texture.width));
	if ((dt.side == 0 || dt.side == 2) && dt.ray_dir.x > 0)
		(*dr).tex_x = dr->texture.width - dr->tex_x - 1;
	if ((dt.side == 1 || dt.side == 3) && dt.ray_dir.y < 0)
		(*dr).tex_x = dr->texture.width - dr->tex_x - 1;
	(*dr).step_f = 1.0 * dr->texture.height / dr->line_height;
}
